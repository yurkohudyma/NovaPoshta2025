...
public class AddresseeService {
    ...
    public Addressee getById(Long addresseeId) {
        return addresseRepository.findById(addresseeId).orElseThrow(
                () -> new EntityNotFoundException
                        ("Addressee " + addresseeId + " NOT FOUND"));
    }

    private Addressee createPersonAddressee () {
        var person = fairy.person();
        return buildAddressee(EntityType.PERSON, createPersonProfile(person));
    }
    private Addressee createCompanyAddressee () {
        Company company = fairy.company();
        return buildAddressee(EntityType.COMPANY, createCompanyProfile(company));
    }
    private Addressee buildAddressee(EntityType type, Profile profile) {
